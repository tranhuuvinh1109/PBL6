...
const EditBlog = ({ id }) => {
	...
	const handleSubmitBlog = async () => {
		...
		if (dataBlog?.title) {
			if (image.preview !== url) {
				url = await uploadFileWithProgress(image, 'images/blog', dataBlog.title, setProgress);
			}
	...
	const renderImage = useMemo(() => {
		return (
			<div className="mt-4">
				{
					image?.preview && <div className="relative">
						<img src={ image.preview } alt='blog' className="p-4" />
						<button className="icon-edit-avatar-blog" onClick={ handleClickClearImage }>
							<FontAwesomeIcon icon={ faXmark } fontWeight={ 900 } fontSize={ 16 } />
						</button>
					</div>
				}
				<div className="flex justify-center mt-3">
					<label htmlFor="image" className="cursor-pointer px-4 py-2 border-red-400 text-red-400 border-2 border-solid rounded-full hover:text-red-600 hover:border-red-600">
						<FontAwesomeIcon icon={ faUpload } className="mr-2" /> Choose file to upload
					</label>
					<input type='file' className="hidden" name="image" id="image" accept=".png, .jpg, .jpeg" onChange={ handleChangeImage } />
				</div>
					...
					<div>
						<div className="max-h-[480px] overflow-y-auto">
							<div className="flex items-center justify-between">
								<label className="w-3/12 m-0 text-lg font-medium" htmlFor='title'>Title :</label>
								<input className='w-9/12 px-4 py-2 border-2 rounded-xl border-slate-400 border-solid' type="text" name="title" id="title" value={ dataBlog?.title } onChange={ handleChangeTitle } />
							</div>
							{
								renderImage
							}
